...
class ControllerModuleCoolfilter extends Controller {
    ...
    public function index($setting) {
        ...
        $data['text_reset_coolfilter'] = $this->language->get('text_reset_coolfilter');
        $data['count_enabled'] = $setting['count_enabled'];
        $data['currency_symbol_left'] = $this->currency->getSymbolLeft($this->currency->getCode());
        $data['currency_symbol_right'] = $this->currency->getSymbolRight($this->currency->getCode());
        $data['count_symbols'] = $this->currency->getDecimalPlace($this->currency->getCode());
        ...
        $coolfilter_group_id = $setting['filter_group'];
        ...
        if (!empty($coolfilter_group_options)) {
            ...
            uasort($data['coolfilters'], array('ControllerModulecoolfilter', 'sortcoolfilters'));

            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/coolfilter.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/module/coolfilter.tpl', $data);
            } else {
                return $this->load->view('default/template/module/coolfilter.tpl', $data);
            }
        }
    }
