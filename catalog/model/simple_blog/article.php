	...
	class ModelSimpleBlogArticle extends Model {
		...
		public function addArticleComment($simple_blog_article_id, $data) {
			...
			if($data['reply_id']) {
				...
				$this->db->query("INSERT INTO `" . DB_PREFIX . "simple_blog_comment` SET simple_blog_article_id='" . (int)$simple_blog_article_id . "', simple_blog_article_reply_id='" . (int)$data['reply_id'] . "', author='" . $this->db->escape($data['comment_name']) . "', comment='" . $this->db->escape($data['comment_text']) . "', status='" . (int)$status . "', date_added=NOW(), date_modified=NOW()");
			} else {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "simple_blog_comment` SET simple_blog_article_id='" . (int)$simple_blog_article_id . "', author='" . $this->db->escape($data['comment_name']) . "', comment='" . $this->db->escape($data['comment_text']) . "', status='" . (int)$status . "', date_added=NOW(), date_modified=NOW()");
			}
		}
			...
			foreach($sql->rows as $row) {
				...
				if($article_info->row) {
					...
					$simple_blog_related_article_data[] = array(
						...
						'image'				=> $article_info->row['featured_image'],
